...
def create_altair_pie(
    chart_df: pl.DataFrame, chart_title: str, value: str, category: str
):
    base_chart = alt.Chart(chart_df, title=chart_title).encode(
        alt.Theta(f"{value}:Q").stack(True), alt.Color(f"{category}:N").legend(None)
    )
...
def create_radial_chart(
    source: pl.DataFrame, values: str, color: str, color_pallete: str
):
    ...
    c2 = base.mark_text(radiusOffset=20).encode(
        text=alt.Text(f"{values}:Q", format=",.2%")
    )
...
def plot_offset_bar(df: pl.DataFrame, xaxis: str, yaxis: str, color: str, xOffset: str):
    offset_bar_chart = (
        alt.Chart(df)
        .mark_bar()
        .encode(alt.X(xaxis), alt.Y(yaxis), color=color, xOffset=xOffset)
    )

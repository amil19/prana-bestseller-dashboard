...
store_counts = (
    ...
    .agg(pl.max("Ordering_Reporting").alias("Stores_Ordering"))
    .collect()
)
...
def get_publisher_df(df, publisher_names):
    df = df.filter(pl.col("PUBLISHER").is_in(publisher_names)).sort(
        "PUBLISHER", descending=False
    )
...
# Select Publishers
pub_names = bs_funcs.create_multiselect(
    base_data.collect(), "PUBLISHER", "Select Publisher"
)
...
cc_data = (
    pub_df_with_dates.group_by("PUBLISHER", "ReportingDate")
    .agg(
...
st.altair_chart(
    bs_plots.plot_offset_bar(
        cc_data,
        xaxis="ReportingDate:O",
        yaxis="CC_Per_Store:Q",
        color="PUBLISHER:N",
        xOffset="PUBLISHER:N",
    )
)
...
launch_df = (
    pub_df_with_dates.drop_nulls("GF_Series_Code")
    .filter(pl.col("ISSUE_NO").is_in([0, 1]))
    .sort("ReportingDate", "MAIN_DESC")
)

# Create benchmark df
benchmarks = (
    pub_df_with_dates.group_by("PUBLISHER", "GF_Series_Code", "Stores_Ordering")
    .agg(pl.sum("ConsumerCopies"))
    .with_columns(
        (pl.col("ConsumerCopies") / pl.col("Stores_Ordering")).alias("Avg_CC_Per_Store")
    )
    .group_by("PUBLISHER")
    .agg(pl.mean("Avg_CC_Per_Store"))
)

benchmarks.write_csv("benchmarks.csv")
